...
#      print 'generate_prod_dedup_valid_uids.py -m <merchant_id> -n <num of users having 3 more views> -p <prefix of uids>'
  ...
  for opt, arg in opts:
    if opt == '-h':
       print 'python map3tree.py -f <parsed xml files> -s <similarity:set or lcs> -l <cluster_limit> -t <cluster_limit_type> -e <example_pic_count>'
       sys.exit()
  ...
  for i in range(len(ind)):
    ...
    if ind[i] not in groups:
      ...
      get_screen_shot(
        url=urls[i], filename=urls[i].replace('/','-')+'.png',
        crop=False, crop_replace=False,
        thumbnail=False, thumbnail_replace=True,
